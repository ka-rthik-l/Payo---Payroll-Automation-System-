...
export const pdfService = {
  ...
  async compilePayslipData(payslipId) {
    const res = await apiClient.get(`/payslips/${payslipId}/pdf-data`);
    return res.data;
  },

  async getPayslipById(payslipId) {
    const res = await apiClient.get(`/payslips/${payslipId}`);
    return res.data;
  },

  async generatePayslipPDF(payslipId) {
    const { blob, filename } = await apiClient.getBlob(`/payslips/${payslipId}/pdf`);
